...
const lay_loai = async () => {
    ...
    // Bắt đầu tạo menu dưới dạng danh sách
    let str: string = `<li><a href="index.html">Trang chủ</a></li>`;
  
    // Thêm các loại sản phẩm vào danh sách
    loai_arr.forEach(loai => {
      str += `<li><a href="sptrongloai.html?id=${loai.id}">${loai.ten_loai}</a></li>`;
    });
  
    // Thêm các mục khác vào danh sách, tất cả đều trong thẻ <li>
    str += `
      <li><a href="#spmoi">Sản phẩm mới</a></li>
      <li><a href="#sphot">Sản phẩm nổi bật</a></li>
      <li><a href="baiviet.html">Bài viết</a></li>
      
    `;
  
    // Trả về danh sách hoàn chỉnh bao bởi thẻ <ul>
    return `<ul>${str}</ul>`;
  };
...
<div class="sp">
    ...
    <p>Khuyến mãi: ${Number(sp.gia_km).toLocaleString("vi")}</p>
     <button class="btsp"> Mua sản phẩm <button/>
</div>`;
} 
